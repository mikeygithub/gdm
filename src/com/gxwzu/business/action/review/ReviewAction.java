...
public class ReviewAction extends BaseAction implements ModelDriven<Review> {
    ...
    private Float addReviewScore(Map<Integer, Float> scoreMap, Review model) {
        ...
        try {
            ...
            for (Iterator<Integer> key = scoreMap.keySet().iterator(); key.hasNext(); weight++) {
                ...
                reviewScore.setOpTime(new Date());

                reviewScore.setScore(scoreMap.get(scoreItemId));
                reviewScoreSerivce.save(reviewScore);//TODO：第二个用户未保存相关分数
    ...
    private Float editReviewScore(Map<Integer, Float> scoreMap, Review model) {
        float totalScore = 0;
        int weight = 1; //权值
        try {
            //遍历评分
            for (Iterator<Integer> key = scoreMap.keySet().iterator(); key.hasNext(); weight++) {
                Integer scoreItemId = key.next();
                ReviewScore reviewScore = reviewScoreSerivce.findById(scoreItemId);
                reviewScore.setReviewId(model.getReviewId());
                reviewScore.setOpTime(new Date());
                reviewScore.setScore(scoreMap.get(scoreItemId));
                reviewScoreSerivce.update(reviewScore);
                totalScore += scoreMap.get(scoreItemId);
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (planProgress != null && planProgress.getStartTime() != null && d.after(planProgress.getStartTime())) {
                try {
                    if (thisStuId != null && thisYear != null && thisId != null && thisReviewType != null) {
                        ...
                        //各项分数
                        ReviewScore reviewScore = new ReviewScore();
                        reviewScore.setYear(thisYear);
                        reviewScore.setReviewId(thisId);
                        review.setReviewScoreList(reviewScoreSerivce.findByExample(reviewScore));
                        logger.info(review);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    public String edit() {
        try {
            if (thisId != null) {
                ...
                //更改分数
                if (scoreIds != null) {
                    Float totalScore = editReviewScore(spiltScore(scoreIds), mReview);
                    if (totalScore != null) {
