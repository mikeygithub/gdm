...
public class SysStudentAction extends BaseAction implements
	...
	public String list() {
		...
		try {
			...
			/******************** 三级查询 end ***********************************/
			pageResult = sysStudentService.find(model, getPage(), getRow());
						...
						for (int j = 1; j <= sheet.getLastRowNum(); j++) {
							...
							if (row.getCell(2) != null) {
								row.getCell(2).setCellType(
										Cell.CELL_TYPE_STRING);
								sysStudent.setStuGrade(row.getCell(2)
										.getStringCellValue());
							}
							...
							if (row.getCell(3) != null) {
								...
								sysMajor = sysMajorService
										.findSysMajorByMajorName(row.getCell(3)
												.getStringCellValue());
								if (sysMajor != null)
									sysStudent
											.setMajorId(sysMajor.getMajorId());
							}
							// 班级
							if (row.getCell(4) != null) {
								row.getCell(4).setCellType(
										Cell.CELL_TYPE_STRING);
								sysClass = sysClassService
										.findSysClassByClassName(row.getCell(4)
												.getStringCellValue());
								if (sysClass != null)
									sysStudent
											.setClassId(sysClass.getClassId());
							}
							// 学号
							...
							List<UserHelp> lHelps = userHelpService
									.findByLoginName(sysStudent.getStuNo());
							if (lHelps.size() > 0) {
								if (exitList == null) {
