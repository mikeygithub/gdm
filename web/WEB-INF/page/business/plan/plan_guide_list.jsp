	...
	<div class="rightinfo">
		...
		<ul class="placeul">
	        	...
	        	<span>学院：<s:select cssClass="dfinput" list="departmentList" 
        		 cssStyle="width: auto;" onchange="$('#form1').submit();"  value="guideCount.deptNumber"
        		 listKey="deptNumber" listValue="deptName"  id="deptNumber" name="guideCount.deptNumber"/></span>		
	        		 </li>
	         <li class="click" >
	        	<span>大类：<s:select cssClass="dfinput" list="categoryList" 
        		 cssStyle="width: auto;" onchange="$('#form1').submit();" value="guideCount.categoryId"
        		 listKey="categoryId" listValue="categoryName"  id="category"  name="guideCount.categoryId"/></span>		
	        		 </li>			  
	         <li class="click" >
	        	<span>教研室：<s:select cssClass="dfinput" list="sysMajorList" 
        		 cssStyle="width: auto;" onchange="$('#form1').submit();" value="guideCount.staffroomId"
        		 listKey="majorId" listValue="staffName" 	id="major"  name="guideCount.staffroomId"/>	</span>		
	        		 </li>	
         ...
         <tbody>
				<s:iterator id="p" value="pageResult.data" status="pp">
					...
					value="${teacherId }"  alt="${guideCountId }"  title="${teacherName}"/> </td>
						...
						<td align="center" id="alreadyStuNum_${teacherId }">${alreadyStuNum}</td>
						...
						<td align="center" width="auto">
							<div name="laypage1.3" class="laypage_main laypageskin_molv" >
								<span class="laypage_total"> <input type="number" style="text-align: center;"
									  ...
									  class="laypage_skip" value="${maxStuNum}" name="maxStuNum" id="maxStuNum_${teacherId }">
									<button type="button" class="laypage_btn" onclick="option(this,${teacherId },'${teacherName}')">
...
function option(name,teacherId,teacherName){
	 ...
	 if(parseInt(already)>parseInt(thisCount)){
		 ...
		 layer.confirm('当前 <font color=blue >'+teacherName+'</font> 老师 <p >可带人数为 '+thisCount+'<p>您确定要修改吗?', {icon: 3, title:'提示'}, function(index){
			  layer.close(index);
